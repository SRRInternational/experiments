...
function analyzeUpdateOperation(updateData, updatedDoc) {
  const changes = {};
  const operatorsHandlers = {
    $set: function (setFields, updatedDoc, changes) {
      Object.keys(setFields).forEach((field) => {
        changes[field] = {
          method: "$set",
          oldValue: updatedDoc._doc[field],
          newValue: setFields[field],
        };
      });
    },
    $pull: function (pullFields, updatedDoc, changes) {
      Object.keys(pullFields).forEach((field) => {
        changes[field] = {
          method: "$pull",
          oldValue: updatedDoc._doc[field],
          newValue: null, // Indicates that the field is removed or set to null
        };
      });
    },
    $push: function (pushFields, updatedDoc, changes) {
      Object.keys(pushFields).forEach((field) => {
        changes[field] = {
          method: "$push",
          oldValue: updatedDoc._doc[field] || [], // Assume it's an array
          newValue: pushFields[field],
        };
      });
    },
  };
  Object.keys(updateData).forEach((operator) => {
    const operatorHandler = operatorsHandlers[operator];
    if (operatorHandler) {
      operatorHandler(updateData[operator], updatedDoc, changes);
    }
  });
  return changes;
}
