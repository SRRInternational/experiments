...
function absorbConfigFile(config, filePath, callback) {
  fs.readFile(filePath, function (err, data) {
    if (err) {
      console.log("error", "Could Not Read Config File :: " + filePath, err.toString());
      return callback(err);
    } else {
      try {
        config.conf = _.extend(config.conf, JSON.parse(data));
      } catch (e) {
        console.log("error", "Invalid Configuration JSON :: " + filePath + "\n" + e.toString());
        return callback(e);
      }
      return callback();
    }
...
Configuration.prototype.load = function (filePath, callback) {
  ...
  if (process.env.serverPort) {
    ...
    console.log("info", "configuration loaded from process.env");
    return callback();
  }
  ...
  fs.exists(filePath, function (exists) {
    if (exists) {
      if (config.configFile !== filePath) {
        // @TODO - notify that config file needs updated
      ...
      absorbConfigFile(config, filePath, function (error) {
        ...
        config.emit("change:config", "configuration");
        console.log("info", "configuration loaded from " + filePath);
        return callback();
      });
    } else {
...
Configuration.prototype.setConfig = function (name, value) {
  if (name && typeof name === "string") {
    ...
    console.log("info", { event: "configuration", message: `global configuration '${name}' changed to '${value}'` });
...
Configuration.prototype.getAllClientSideConfigs = function () {
  ...
  return ALLOWED_CLIENT_SIDE_KEYS.reduce(
    (result, key) => {
