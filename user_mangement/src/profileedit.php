...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Check if image is uploaded and upload it
    $image = $_FILES["image"];
    $bnr_image = $_FILES["bnr_image"];
    $imagepath = '';
    $bnrImagePath = '';

    if (!empty($image["name"])) {
        $imageUploadResult = $userHandler->uploadImage($image);
        if ($imageUploadResult['success']) {
            $imagepath = $imageUploadResult['file_path'];
        } else {
            // Handle image upload error
            echo $imageUploadResult['message'];
            exit();
        }
    } else {
    ...
    if (!empty($bnr_image["name"])) {
        $bnrImageUploadResult = $userHandler->uploadImage($bnr_image);
        if ($bnrImageUploadResult['success']) {
            $bnrImagePath = $bnrImageUploadResult['file_path'];
        } else {
            // Handle bnr image upload error
            echo $bnrImageUploadResult['message'];
            exit();
        }
    } else {
        $bnrImagePath = $userInfo['bnr_image'];
    }
    ...
    if ($userHandler->updateUser($username, $dob, $age, $profession, $email, $imagepath, $bnrImagePath, $firstname, $lastname, $mobile, $country, $bio)) {
            ...
            <main class="p-4">
                <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
                    ...
                    <div class=" w-full mx-auto rounded-lg border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ">
                        ...
                        echo $mustache->render(file_get_contents('./templates/profile_edit.mustache'), $data);
                        ?>
