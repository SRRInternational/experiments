define([
  ...
  var PageLevelProgress = Backbone.Controller.extend({
    ...
    initialize: function() {//will listen to data ready event
      Adapt.on({
    ...
    getCourseConfig: function() {// will retrieve course config
      return Adapt.course.get('_pageLevelProgress');
    },

    onDataReady: function() {//on data ready event
    ...
    setUpEventListeners: function() {
      ...
      this.listenTo(Adapt, {
        'menuItemView:postRender': this.renderMenuItemIndicatorView,// on menu items render
    ...
    onCompletionChange: function(event) {//listening to change event
    ...
    renderHeaderIndicatorView: function(view) {//on view ready: menu, menuitem, article, block, component 
    ...
    renderMenuItemIndicatorView: function(view) {//on menuItemView:postRender
    ...
    _getMenuItemCompletionPercentage: function() {// will call calculatePercentageComplete
